...
def allocate_public_code() -> str:
    """Kod uczestnika unikalny w skali bazy. Unikalność gwarantuje też constraint w bazie."""
    for _ in range(PUBLIC_CODE_MAX_ATTEMPTS):
        code = generate_public_code()
        if not Participant.objects.filter(public_code=code).exists():
            return code
    raise DomainError(
