...
| Źródło | Finding | Plan |
|---|---|---|
| T-02 | `district` członka komitetu jest samodeklarowany; reguła konfliktu interesów (T-05) na nim polega | **zamknięte decyzją organizatora** („województwo nie ma znaczenia w przypadku członków komitetu, może być opcjonalne”): województwo członka komitetu jest opcjonalne, konfliktem jest wyłącznie województwo **równe** województwu uczestnika na etapie wojewódzkim, a `district_verified` niczego nie bramkuje (mówi tylko, czy wartość pochodzi od organizatora, czy z samodeklaracji). Wcześniej: zamknięta lista 16 województw (`accounts.Voivodeship`, slug ASCII) + migracja danych `accounts.0007` – wolny tekst nie ma już jak wejść |
...
`entry_totals` jako osobna tabela, TTL i rotacja tokenu DRF,
...
| reset hasła | ~~Wysyłka listu synchroniczna w żądaniu `POST /password-reset/` (`EMAIL_TIMEOUT=10` ograniczał tylko czas zajęcia workera; gdy `mail` nie odpowiadał, POST czekał do timeoutu, a czas odpowiedzi zdradzał, czy konto istnieje)~~ **zamknięte** | zamknięte: `QueuedPasswordResetForm` (`apps/accounts/password_reset.py`) – te same szablony, kontekst i token co `PasswordResetForm` Django, ale list idzie przez `send_mail_task` (kolejka `mail`, nowy argument `html_message`) po commicie; obie drogi – samoobsługowa i koordynatora (`/coordinator/accounts/<pk>/password-reset/`) – przez ten sam formularz. Błąd kolejkowania połykany i logowany po kluczu konta (bez wyroczni enumeracji). Testy: `apps/web/tests/test_password_reset_queue.py` |
...
| OAuth | ~~Świadomie przyjęte~~ **zamknięte:** przyczyną było to, że rejestracji hasłem nie poprzedzała weryfikacja adresu, więc allauth czyścił hasło konta o niepotwierdzonym adresie (`wipe_password`) – checklista § 3.2.8 | zamknięte: aktywacja konta linkiem z listu (`apps/accounts/activation.py`, README § 5.1). `mark_activated` zapisuje też `allauth.EmailAddress(verified=True)`, więc auto-connect Google nie ma już czego czyścić |
| prod 2026-09-09 | **Incydent:** po dobie pracy `web` trzymał 92 bezczynne połączenia do Postgresa (`CONN_MAX_AGE=60` pod ASGI – połączenie przypięte do wątku żądania, który ginie bez `close_old_connections`), Postgres odpowiadał „too many clients already” i każda strona dawała 500 | zamknięte: `CONN_MAX_AGE=0` (`DB_CONN_MAX_AGE` w env), potem WSGI + `gthread` (v0.31.0). Dług spłacony: pula psycopg w `web` (`OPTIONS["pool"]`, `psycopg[pool]`, `config/dbpool.py`; `worker`/`beat` bez puli – Celery prefork zamyka pulę przy każdym zadaniu), `application_name` per usługa i alarm zajętości `max_connections` (`apps/core/dbconnections.py`: poziom w `/healthz/` i `/status.json`, list watchdoga od 80 %, `manage.py db_connections`) – `OPERACJE.md` § 11.2 |
