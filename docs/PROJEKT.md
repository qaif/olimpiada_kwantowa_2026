...
- Wynik pokazany uczestnikowi: własne punkty, `comment_for_participant`, adnotacje oznaczone jako publiczne. `comment_internal` i tożsamość recenzenta nie są ujawniane.
- Publikacja: `ResultsPublication.snapshot` jest zamrożony i zanonimizowany według `anonymization`. Pełne nazwiska tylko dla laureatów finału i tylko po zgodzie w profilu (RODO: minimalizacja + zgoda).
