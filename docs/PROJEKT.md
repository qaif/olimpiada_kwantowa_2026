...
`Problem.statement_pdf` ma jawnie wskazany `storage=apps.competitions.storage.private_media_storage`; gdyby został na `default`, treść zadania byłaby czytelna anonimowo przed otwarciem etapu. Plik nadal serwuje `ProblemStatementView` (`FileResponse` ze strumienia z storage, 404 przed `opens_at`) – nigdy bezpośredni URL obiektu. Bucket `submissions` pozostaje dla Wagtaila niedostępny: Wagtail używa wyłącznie aliasu `default`. Lokalnie i w testach `private_media` to `PrivateMediaFileSystemStorage` – podkatalog `MEDIA_ROOT/private`, a nie ten sam katalog co `default`; dzięki temu reguła „`statement_pdf` nie idzie przez `default`” daje się sprawdzić ścieżką pliku, a nie tylko nazwą aliasu. Pliki zapisane przed T-09 przenosi idempotentna komenda `manage.py migrate_statements` (`--dry-run` pokazuje plan; źródła nie kasuje).

**Poświadczenia są rozdzielone razem z bucketami.** Sam podział aliasów `STORAGES` nie jest zabezpieczeniem, dopóki wszystkie storage'e chodzą na koncie administracyjnym MinIO: każdy błąd w ścieżce redakcyjnej (a tam pliki przychodzą od człowieka) sięgałby wtedy także prac uczestników. `minio-init` tworzy dwa konta serwisowe z polityką na jeden bucket; `S3_PUBLIC_*` dostaje alias `default` (Wagtail), `S3_PRIVATE_*` – alias `private_media` **oraz** `S3SubmissionStorage` (także klient podpisujący presigned URL-e: podpis dziedziczy uprawnienia klucza). Brak zmiennych oznacza fallback na `MINIO_ROOT_*` dla zgodności wstecznej – z ostrzeżeniem w logu startowym, bo cichy fallback wyglądałby jak rozdzielona instalacja, którą nie jest.

**Dokumenty Wagtaila: `WAGTAILDOCS_SERVE_METHOD = "serve_view"`.** Domyślnie Wagtail przy zdalnym storage wybiera `redirect` i oddaje 302 na adres bucketu; przy `serve_view` streamuje plik z aplikacji, więc kontrola dostępu (ograniczenia widoczności kolekcji) i treść idą tą samą drogą, a URL obiektu nie zostaje w historii przeglądarki ani w logach proxy. **Uwaga – to nie czyni dokumentu tajnym:** obiekt dalej leży w buckecie `public-media` z polityką `download`, więc kto zna bezpośredni URL, przeczyta go anonimowo. Ograniczenie kolekcji chowa dokument przed listą i przed widokiem, nie przed bucketem. Materiały, które naprawdę nie mogą wyciec (klucze odpowiedzi, protokoły komisji), idą do `private_media` i przez widok aplikacji – **nigdy** do dokumentów Wagtaila. Przeniesienie samych dokumentów do prywatnego bucketu wymagałoby własnego modelu `WAGTAILDOCS_DOCUMENT_MODEL` z podmienionym storage pola – uznane za nieproporcjonalnie inwazyjne wobec ryzyka, decyzja do rewizji, gdy pojawi się realny materiał tej klasy.

**Osadzenia (`EmbedBlock`).** `WAGTAILEMBEDS_FINDERS` zawężone do dwóch dostawców oEmbed: YouTube i Vimeo. Domyślna lista Wagtaila to ponad 70 serwisów – każdy z nich to obcy `<iframe>` na naszej domenie i obce żądanie z przeglądarki czytelnika. Adres spoza listy kończy się `EmbedUnsupportedProviderException` już w edytorze. Ta sama lista stoi w `frame-src` polityki CSP (`apps/web/middleware.py`): finder decyduje, co redaktor może wstawić, CSP – co przeglądarka wykona.

**CSP.** Panel redakcyjny i panel Django dostają osobną, luźniejszą politykę z `'unsafe-inline'` (i bez nonce'a – nonce unieważniłby `'unsafe-inline'`), bo Wagtail i panel Django wstrzykują skrypty inline z własnych szablonów. Wszystkie pozostałe ścieżki, w tym całe drzewo stron CMS, zachowują politykę nonce-only bez `'unsafe-inline'`/`'unsafe-eval'` dla `script-src`. Wybór polityki idzie po `resolver_match` (przestrzeń nazw `admin`, nazwa widoku `wagtailadmin_…`), a dopiero na końcu po prefiksie ścieżki wyprowadzonym z `reverse()` – nie po literale `/cms/`, żeby przeniesienie panelu w `config/urls.py` nie zostawiło luźnej polityki pod starym adresem. `img-src` i `media-src` zawierają origin `S3_PUBLIC_ENDPOINT_URL`: obrazy i renditions Wagtaila stoją w publicznym buckecie i są linkowane bezpośrednio, więc bez tego wpisu produkcja blokuje każdą ilustrację redakcyjną (dev tego nie pokaże – tam storage jest lokalny). Treści redaktorów renderują się przez `|richtext` i `{% include_block %}` (whitelist Wagtaila); w szablonach `cms/` nie ma ani jednego `|safe`.

**Slugi zarezerwowane.** Wagtail jest catch-allem w korzeniu, ale w `config/urls.py` stoi ostatni – strona CMS o slugu `login` na drugim poziomie drzewa miałaby adres `/login/` i byłaby po prostu nieosiągalna (redaktor widzi „opublikowano”, czytelnik – formularz logowania). Bazowa klasa `apps.cms.models.CMSPage` odrzuca w `clean()` slugi należące do aplikacji: `login, logout, me, review, coordinator, appeals, results, documents, cms, admin, healthz, api, static, media, register`. Ograniczenie dotyczy wyłącznie drugiego poziomu – `/aktualnosci/login/` jest w porządku.

**Wydajność stron listowych.** `ResultsPage` czyta publikacje jednym zapytaniem (`ResultsPublication` + `select_related("stage__edition")`), a pełne tabele renderuje wyłącznie dla bieżącej edycji; starsze roczniki zostają odnośnikiem do `/results/<id>/`, bo snapshot finału to tysiące wierszy i sklejanie ich w jeden dokument dawałoby stronę rosnącą z każdą edycją. `ArchiveEditionPage` podaje dokumenty przez `select_related("document")`. Liczba zapytań obu stron jest stała względem liczby wierszy – jest to testowane.
...
- `Review.score ∈` skali obowiązującej dla zadania: `Problem.scoring_values`, a w ich braku `ScoringScale.values` etapu. Walidacja w `clean()` i w serwisie zapisu (`grading.services.allowed_scores`).
...
- Ślepota rundy 1 dotyczy **czasu przed kompletem ocen**: dopóki którakolwiek recenzja rundy 1 nie jest `SUBMITTED`, recenzent nie widzi cudzych punktów. Po komplecie (albo gdy praca jest w `MODERATION`/oceniona) panel pokazuje mu punkty i komentarz publiczny pozostałych recenzentów oraz wspólny wątek notatek (`grading.ReviewNote`) — **bez tożsamości autorów** („Recenzent B”), tak jak materiał rozjemczy rundy 2. Ukrywanie ocen po ich wystawieniu nie chroni już niezależności, a odbiera komitetowi możliwość uzgodnienia stanowiska bez posiedzenia. Wątek zamyka się razem ze sprawą (praca `FINAL` albo ogłoszone wyniki) i nie jest widoczny dla uczestnika.
...
- **Retencja (art. 5 ust. 1 lit. e RODO)**: dane uczestników edycji mają termin. Liczy go `Edition.data_retention_months` (domyślnie 24) od **ostatniego deadline'u etapu** tej edycji; po jego upływie zadanie dobowe `apps.accounts.retention` anonimizuje konta tą samą funkcją, co żądanie z art. 17 (`anonymise_account`) – zostaje pseudonimowy `public_code`, województwo i dokumentacja zawodów. Automat nie rusza kont startujących w edycji późniejszej, kont z nierozstrzygniętą reklamacją, kont z etapem bez ogłoszonych wyników ani edycji bieżącej (bezwarunkowo, nie przez sam termin). Ślad: `account.anonymised_by_retention` z identyfikatorem edycji. Dry-run przed skutkiem: `manage.py retention_report` i `/coordinator/retention/`.
- **Przenoszenie danych (art. 20 RODO)**: `/account/export/` oddaje paczkę ZIP z `dane.json` (konto, profile ról, zgody wraz z wersjami dokumentów, zgłoszenia do etapów, metryka prac z `sha256`, własny wiersz ogłoszonej tabeli, oceny końcowe z komentarzami dla uczestnika, preferencje) oraz z plikami, które uczestnik wgrał. Zakres jest wyznaczony tym, co widzi w panelu (`apps.results.feedback`): nigdy `comment_internal`, nigdy cudze wiersze tabeli, nigdy poświadczenia. Limit jeden eksport na 10 minut **per konto**; audyt `account.exported` bez treści. Tę samą paczkę wydaje koordynator na wniosek złożony poza serwisem (`account.exported_by_coordinator`).
- **Rejestr czynności przetwarzania (art. 30 RODO)** jest danymi w kodzie (`apps/accounts/processing_register.py`), a nie arkuszem: zmiana przetwarzania jest zmianą w repozytorium i przechodzi przez recenzję, a treść rejestru sprawdza test. Renderuje go `/coordinator/processing-register/` (HTML + CSV); dane administratora dokłada `cms.SiteSettings`.
