...
Zamyka dług opisany w 3.2.8: do tej zmiany rejestracji hasłem nie poprzedzała żadna weryfikacja
adresu, więc konto dawało się założyć na cudzy adres. Wszystko poniżej pilnują testy
z `apps/accounts/tests/test_activation.py`, `apps/web/tests/test_activation.py`
i `apps/accounts/tests/test_purge_unactivated.py`. Konfiguracja: `README.md` § 5.1.

| # | Pozycja | Status | Gdzie / czym sprawdzone |
|---|---|---|---|
| 3.1a.1 | Konto z rejestracji nie loguje się przed potwierdzeniem adresu | ✔ | `accounts.services._create_user(is_active=False)` na obu drogach rejestracji (uczestnik, komitet na kod). Odrzuca je `ModelBackend`, czyli **ogólnym** komunikatem „nieprawidłowy e-mail lub hasło” – strona logowania nie zdradza, że konto istnieje i czeka. Testy: `::test_inactive_account_cannot_authenticate`, `::test_an_activated_account_can_log_in_and_an_inactive_one_cannot`. |
| 3.1a.2 | Token podpisany, wiązany z parą (pk, adres), ważny 4 h, jednorazowy | ✔ | `apps.accounts.activation` (`django.core.signing` na `TimestampSigner`, sól własna dla każdego przeznaczenia). Jednorazowość bierze się ze stanu (`email_verified_at is None`), nie z tabeli tokenów. Testy: `::test_the_link_activates_the_account_exactly_once`, `::test_a_token_older_than_the_window_is_refused`, `::test_a_token_stops_matching_after_the_address_changes`, `::test_a_made_up_token_is_refused`. |
| 3.1a.3 | Aktywacja nie loguje automatycznie | ✔ | `ActivateAccountView` pokazuje stronę „Konto aktywne – zaloguj się”. Dostęp do skrzynki pocztowej nie zamienia się jednym kliknięciem w sesję (ta sama reguła, co 3.1.5). Test: `::test_the_link_from_the_message_activates_the_account`. |
| 3.1a.4 | Ponowna wysyłka bez enumeracji kont | ✔ | Jeden komunikat (`RESEND_MESSAGE`) i to samo przekierowanie dla adresu istniejącego, nieistniejącego i już aktywnego. Testy: `::test_resend_answers_identically_for_a_known_and_an_unknown_address`, `::test_resend_sends_nothing_for_an_unknown_or_already_active_account`. |
| 3.1a.5 | Limit na ponownej wysyłce | ✔ | Scope `password_reset` (5/h) – **wspólny** z resetem hasła, bo oba formularze wysyłają list na adres podany przez anonima. Konsumuje każdy POST. Test: `::test_resend_is_throttled_with_the_password_reset_budget`. |
| 3.1a.6 | List bez danych osobowych | ✔ | Poza adresem odbiorcy (i tak w nagłówku `To:`) w treści nie ma imienia, nazwiska ani szkoły; temat jest stały. Test: `::test_registration_creates_an_inactive_account_and_sends_the_link`. |
| 3.1a.7 | Dostawca, któremu ufamy, zastępuje nasz list | ✔ | `adapters.sociallogin_email_verified` czyta **odpowiedź dostawcy o tym adresie**, nie konfigurację: Google z `email_verified` → konto aktywne od razu, Facebook (`VERIFIED_EMAIL: False`) → zwykła aktywacja. Testy: `::test_provider_verified_address_means_an_active_account_without_our_link`, `::test_provider_without_a_verified_address_goes_through_activation`. |
| 3.1a.8 | Ręczna aktywacja przez koordynatora jest odróżnialna w audycie | ✔ | `account.activated_by_coordinator` obok `account.activated` – przy linku dowodem jest dostęp do skrzynki, tutaj decyzja człowieka. Sekcja panelu jest obejściem na czas braku SPF/DKIM (`README.md` § 4.2). Testy: `::test_coordinator_activates_an_account_manually_and_it_is_audited`, `::test_the_waiting_list_is_closed_to_everyone_but_the_coordinator`. |
| 3.1a.9 | Konta nieaktywowane są kasowane po 4 h | ✔ | `apps.accounts.tasks.purge_unactivated_accounts` (beat co 15 min, komenda `manage.py purge_unactivated_accounts`). Kasuje **wyłącznie** `is_active=False` + `email_verified_at IS NULL` + starsze niż okno; konto z odwołaniem w dokumentacji zawodów zostaje i idzie do logu. Jeden wpis audytowy na przebieg, wyłącznie liczby. Testy: `test_purge_unactivated.py` (cały plik). |
| 3.1a.10 | Migracja nie odcina istniejących kont | ✔ | `accounts.0010` ustawia `email_verified_at = date_joined` każdemu kontu `is_active=True`; konto zablokowane (`is_active=False`) zostaje bez zmian i **nie** trafia pod kosiarkę, bo ta wymaga też pustego `email_verified_at`. Test: `apps/accounts/tests/test_migrations.py`. |

### 3.1b Własne konto: zmiana adresu e-mail i usunięcie konta

Testy: `apps/web/tests/test_profile.py`, `apps/web/tests/test_account_deletion.py`. Opis:
`README.md` § 5.2.

| # | Pozycja | Status | Gdzie / czym sprawdzone |
|---|---|---|---|
| 3.1b.1 | Adres e-mail zmienia się dopiero po potwierdzeniu na **nowej** skrzynce | ✔ | `accounts.profile.request_email_change` wysyła link, `confirm_email_change` zmienia adres. Gdyby zmiana działała od razu, literówka zamykałaby drogę powrotu (login i reset idą przez adres), a przejęta sesja przenosiłaby konto jednym POST-em. Testy: `::test_the_address_changes_only_after_the_link_is_clicked`, `::test_a_made_up_confirmation_link_changes_nothing`. |
| 3.1b.2 | Token zmiany niesie **oba** adresy – stary link jest martwy | ✔ | Zgodność starego adresu z bazą unieważnia linki z wcześniejszych prób. Test: `::test_a_confirmation_link_goes_stale_after_a_second_change`. |
| 3.1b.3 | Unikalność adresu bez względu na wielkość liter | ✔ | `_assert_email_free` (`iexact`) sprawdzany **dwukrotnie**: przy wniosku i przy potwierdzeniu – między listem a kliknięciem ktoś mógł zająć adres. Constraint `accounts_user_email_ci_uniq`. Test: `::test_an_address_already_taken_is_refused_case_insensitively`. |
| 3.1b.4 | Stary adres jest powiadamiany o przeniesieniu konta | ✔ | To jedyny sygnał dla właściciela skrzynki, który o zmianę nie prosił. Test: `::test_the_address_changes_only_after_the_link_is_clicked`. |
| 3.1b.5 | Limit na formularzu zmiany adresu | ✔ | Scope `password_reset` – formularz wysyła list na adres podany przez użytkownika. |
| 3.1b.6 | Usunięcie konta wymaga potwierdzenia tożsamości | ✔ | Hasło, a dla konta bez użytecznego hasła (Google/Facebook) przepisanie własnego adresu. Bez tego każda niezamknięta sesja w cudzej przeglądarce byłaby jednym kliknięciem od usunięcia danych. Testy: `::test_a_wrong_password_changes_nothing`, `::test_an_account_without_a_usable_password_confirms_with_its_address`. |
| 3.1b.7 | Konto ze śladem w zawodach jest anonimizowane, nie kasowane | ✔ | `accounts.profile.anonymise_account`: PII znika, `public_code` i dokumentacja zostają, zgody dostają `withdrawn_at`, zamrożony snapshot wyników jest nietknięty. Zamyka dług 7.8 dla obu ról. Testy: `::test_a_participant_of_the_competition_is_anonymised_not_deleted`, `::test_the_published_snapshot_is_left_untouched`, `::test_a_reviewer_with_reviews_is_anonymised_too`. |
| 3.1b.8 | Usunięcie odbiera **wszystkie** poświadczenia i sesje | ✔ | Hasło, token DRF, `SocialAccount`, `EmailAddress`, wiersze sesji. Testy: `::test_deleting_closes_every_session_of_the_account`, `::test_anonymisation_drops_the_provider_links_and_the_api_token`. |
| 3.1b.9 | Koordynator nie usuwa własnego konta z panelu | ✔ | `delete_own_account` odmawia: to jedyne wejście do prowadzenia edycji, a `InvitationCode.created_by` jest na nim `PROTECT`. Test: `::test_the_coordinator_cannot_delete_their_own_account_here`. |
| 3.1b.10 | Audyt usunięcia bez danych osobowych | ✔ | `account.deleted` / `account.anonymised` z samym identyfikatorem; `participant.profile_updated` niesie **nazwy** zmienionych pól, a wartości wyłącznie dla danych nieidentyfikujących (województwo, klasa, rocznik). Testy: `::test_an_account_without_a_footprint_is_deleted_completely`, `::test_the_audit_entry_names_the_changed_fields_without_the_values`. |
| 3.1b.11 | Okręg członka komitetu nie jest samoobsługowy | ✔ | `/account/profile/` edytuje wyłącznie imię i nazwisko; potwierdzony okręg (podstawa reguły konfliktu interesów) zmienia koordynator. Test: `::test_the_committee_district_is_read_only_on_that_page`. |
...
| 7.8 | Usunięcie/anonimizacja konta na żądanie (art. 17 RODO) | ✔ | `Review.reviewer` i `AppealDecision.committee` mają `PROTECT`, więc konta z oceną **nie kasujemy** – jest anonimizowane (`accounts.profile.anonymise_account`). Ta sama reguła dotyczy uczestnika ze zgłoszeniem lub pracą; konto bez śladu w dokumentacji znika w całości. Szczegóły i testy: 3.1b.7, `README.md` § 5.2. |
...
| 8.6 | Enumeracja kont przez `EMAIL_TAKEN` | ⚠ `low`, zaakceptowane | Świadomy kompromis UX + limit 10/h/IP. `BACKLOG.md` (dług T-02). Reset hasła i ponowna wysyłka linku aktywacyjnego tej dziury **nie** mają – patrz 3.1.1 i 3.1a.4. |
...
| 8.8 | Wspólny budżet listów wysyłanych na cudzy adres | ✔ | Scope `password_reset` obsługuje **trzy** formularze: reset hasła, ponowną wysyłkę linku aktywacyjnego (`/activate/resend/`) i zmianę adresu e-mail (`/account/email/`). Osobne budżety znaczyłyby, że ten sam adres da się zasypać listami przez drugi formularz. Patrz 3.1a.5, 3.1b.5. |

### 8.1 Publiczne formularze rejestracji: CAPTCHA, pułapka, czas wypełniania

Limit prób ogranicza **liczbę** zgłoszeń z jednego adresu, ale nie podnosi kosztu jednego z nich –
dziesięć kont na godzinę z każdego adresu w botnecie to nadal seryjna rejestracja. Stąd trzy
niezależne warstwy w `apps/web/captcha.py`, wpięte w `ParticipantRegisterForm`
i `CommitteeRegisterForm`. Wszystko poniżej pilnuje `apps/web/tests/test_antispam.py`.

| # | Pozycja | Status | Gdzie / czym sprawdzone |
|---|---|---|---|
| 8.1.1 | CAPTCHA bez podmiotu trzeciego i bez ciasteczek | ✔ | `django-simple-captcha`: obrazek rysuje Pillow w naszym procesie, wyzwanie leży w `captcha.CaptchaStore`, adres jest nasz (`/captcha/image/<klucz>/`), więc mieści się w `img-src 'self'` – do CSP nie doszedł ani jeden host. Żadnego skryptu pakietu nie montujemy (refresh przez AJAX wymagałby jQuery i rozluźnienia `script-src`); nowy obrazek daje przeładowanie strony. Powód decyzji: obietnica z `/dokumenty/cookies/`. Testy: `::test_captcha_image_comes_from_our_own_address_and_passes_the_policy`, `::test_registration_page_renders_the_captcha_after_the_consents`. |
| 8.1.2 | Wyzwanie jednorazowe i wygasające | ✔ | `CAPTCHA_CHALLENGE_FUNCT = math_challenge` (działanie arytmetyczne – czytelniejsze od losowych liter), `CAPTCHA_TIMEOUT = 10` min; poprawna odpowiedź **kasuje** wiersz, więc tej samej pary (klucz, wynik) nie da się użyć dwa razy. Test: `::test_wrong_answer_is_rejected_and_the_right_one_passes`. |
| 8.1.3 | Pułapka ukryta stylem, nie typem pola | ✔ | Pole `website` z klasą `hp-field` (CSS przesuwa je poza ekran) – pole `type="hidden"` byłoby dla bota sygnałem „nie wypełniaj”. Odrzucenie jest ciche: jeden ogólny komunikat, bez nazwania reguły. Testy: `::test_honeypot_is_hidden_by_stylesheet_and_not_by_input_type`, `::test_filled_honeypot_is_rejected_without_saying_why`, `::test_committee_honeypot_is_rejected_before_the_invitation_code`. |
| 8.1.4 | Minimalny czas wypełniania na **podpisanym** znaczniku | ✔ | Ukryte `form_ts` = `django.core.signing.Signer(salt=…)` po sekundach epoki; POST szybszy niż `ANTISPAM_MIN_FILL_SECONDS` (3 s) albo z podpisem, który nie pasuje, jest odrzucany. Bez podpisu bot przepisałby znacznik na dowolną wartość z przeszłości. Testy: `::test_submission_faster_than_the_threshold_is_rejected`, `::test_timestamp_without_our_signature_is_rejected`. |
| 8.1.5 | Pola pomocnicze nie docierają do serwisu | ✔ | `CaptchaFormMixin.clean()` zdejmuje `captcha`/`website`/`form_ts` z `cleaned_data` – widoki wołają serwisy przez `**form.cleaned_data`, więc nadmiarowy klucz byłby `TypeError`. Testy: `::test_helper_fields_do_not_reach_the_service`, `apps/web/tests/test_consents.py::test_consent_fields_do_not_leak_into_the_service_kwargs`. |
| 8.1.6 | Obejście wyłącznie pod `E2E_MODE` | ✔ świadome | `CAPTCHA_TEST_MODE` (odpowiedź `PASSED`) i zerowy próg czasu włączają się w `config/settings/test.py` oraz przy `E2E_MODE=1` – tej zmiennej w produkcji nie ustawia się nigdy (komentarz przy jej definicji w `config/settings/base.py`; nakładka `docker-compose.dev.yml` ustawia ją dla usługi `web`, czyli także dla środowiska developerskiego). Weryfikacja na produkcji: `docker compose exec web python -c "from django.conf import settings; print(settings.E2E_MODE, settings.ANTISPAM_MIN_FILL_SECONDS, getattr(settings, 'CAPTCHA_TEST_MODE', False))"` → `False 3 False`. |
| 8.1.7 | Brak wersji dźwiękowej wyzwania | ⚠ `low`, świadome | `CAPTCHA_FLITE_PATH` nieustawione: `flite` to kolejny binarny pakiet w obrazie, a synteza po angielsku nie pomogłaby polskiemu uczestnikowi. Ograniczenie jest **jawne w interfejsie** – podpowiedź pod polem podaje `contact@qaif.org` i tą drogą konto zakłada organizator. Alternatywa (wyzwanie tekstowe dla czytników ekranu) – `BACKLOG.md`. |
| 8.1.8 | Rejestracja przez Google/Facebooka bez CAPTCHY | ✔ świadome | `SocialParticipantSignupForm` jej nie ma: formularz stoi **za** uściskiem dłoni OAuth, więc bot musiałby najpierw założyć konto u dostawcy i przejść jego własne zabezpieczenia. Test: `::test_social_signup_has_no_captcha`. |
